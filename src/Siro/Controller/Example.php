...
class Example
{
    static public function mountOn($factory, $app)
    {
        $factory->match('/', function() use ($app) {
            $app['session']->getFlashBag()->add('warning', 'Warning flash message');
            $app['session']->getFlashBag()->add('info', 'Info flash message');
            $app['session']->getFlashBag()->add('success', 'Success flash message');
            $app['session']->getFlashBag()->add('error', 'Error flash message');

            return $app['twig']->render('example/index.html.twig');
        })->bind('homepage');

        $factory->match('/login', function(Request $request) use ($app) {
            ...
            return $app['twig']->render('example/login.html.twig', array(
                'form'  => $form->createView(),
                'error' => $app['security.last_error']($request),
            ));
        })->bind('login');

        $factory->match('/doctrine', function() use ($app) {
            return $app['twig']->render(
                'example/doctrine.html.twig',
                array(
        ...
        $factory->match('/form', function(Request $request) use ($app) {
            ...
            return $app['twig']->render('example/form.html.twig', array('form' => $form->createView()));
        })->bind('form');

        $factory->match('/logout', function() use ($app) {
            $app['session']->clear();

            return $app->redirect($app['url_generator']->generate('homepage'));
        })->bind('logout');

        $factory->get('/page-with-cache', function() use ($app) {
            $response = new Response($app['twig']->render('example/page-with-cache.html.twig', array('date' => date('Y-M-d h:i:s'))));
            $response->setTtl(10);
